...
class AssociateTotalDiscountsSetAction extends BaseAssociateAction
{
    ...
    protected function createItem($list)
    {
        ...
        $totalDiscountsSet = $this->model->totalDiscountsSet->fill($values);

        foreach ($this->associateActions() as $attachActions) {
            (new $attachActions($list))->execute($totalDiscountsSet);
        }

        $totalDiscountsSet->save();

        $this->model->totalDiscountsSet()->associate($totalDiscountsSet);
    }
